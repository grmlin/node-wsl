 ...
 * @property {boolean} default true if it's the default distribution
 * @property {boolean} running true if the distribution is currently running
 * @property {boolean} stopped true if the distribution is currently stopped
 */
...
const parse = (listResponse) => {
  ...
  const wslStatus = lines
    ...
    .map((line) => {
      ...
      const defaultD = columns[INDEX_DEFAULT].length > 0;
